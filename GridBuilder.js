...
var GridBuilder = (function () {
    ...
    function onPathUpdate() {
        ...
        var score = 0;
        var $tr = $('<tr />');
        console.log('alignedTop: '+alignedTopSeq)
        ...
        $table.append($tr);
        //**************
        score = mPathTable[mlastX][mlastY];
    ...
    mSelf = {
        ...
        highlightOptimal: function() {
            ...
            if(mIsGlobal){
                ...
                while (currentX > -1 && currentY > -1) {
                   ...
                   if(mPathTable[currentX][currentY]==0)
                         break;
        ...
        rebuildTable: function(domContainer, resultContainer, matchScore, mismatchScore, gapScore, seqSide, seqTop) {
            ...
            var width = mTopSequence.length + 1;
            var height = mSideSequence.length + 1;
            //for get last position
            var lastX = width-1;
            var lastY = height-1;
            var lastVal = (height-1)*gapScore;
            ...
            for (var i = 0; i < width; i++) {
                ...
                for (var j = 0; j < height; j++) {
                    ...
                    */
                    ...
                    //Fitting: update max of the last row
                    if(mIsFitting && j == height-1){
                        //update last pos
                        if(mPathTable[i][j]>= lastVal){
                            lastX= i;
                            lastY =j;
                            lastVal = mPathTable[i][j];
                        }
                    }else if(!mIsGlobal && !mIsFitting){
                        //update last pos
                        if(mPathTable[i][j]>= lastVal){
                            lastX= i;
                            lastY =j;
